#ifndef   _PERF_DEFINE_H_  
#define   _PERF_DEFINE_H_
...
#define Get_ProcTag_Priv(tag, priv) asm volatile( \
    Get_ProcTag("t0")    \
    "mv %[value1], t0  \n\t"  \
    Get_Maxpriv("t0")    \
    "mv %[value2], t0  \n\t"  \
    : [value1]"=r"(tag), [value2]"=r"(priv)\
    :  \
);
...
#define SetProcTag(ptag) asm volatile( \
    "mv t0, %[value]   \n\t"  \
    Set_ProcTag("t0")        \
    : \
    :[value]"r"(ptag)  \
